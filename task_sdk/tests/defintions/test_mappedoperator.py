...
def test_mapped_render_template_fields_validating_operator(tmp_path, mock_supervisor_comms):
    ...
    with DAG("test", template_searchpath=tmp_path.__fspath__()):
        task1 = BaseOperator(task_id="op1")
    ...
    assert mapped.partial_template == "a", "Should be templated!"
    assert mapped.partial_static == "{{ ti.task_id }}", "Should not be templated!"
    assert mapped.map_template == "{{ ds }}", "Should not be templated!"
    assert mapped.map_static == "{{ ds }}", "Should not be templated!"
    assert mapped.file_template == "loaded data", "Should be templated!"
