...
class UnmappableOperator(TypeError):
...
class MappedOperator(AbstractOperator):
    ...
    def deps_for(operator_class: type[BaseOperator]) -> frozenset[BaseTIDep]:
        # TODO: Task-SDK: Do we even need definition-time deps, since they run on the scheduler, shouldn't
        # this might have to be removed.
        operator_deps: frozenset[BaseTIDep] = operator_class.deps  # type: ignore[attr-defined]
        if not isinstance(operator_deps, Set):
