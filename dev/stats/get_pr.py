#!/usr/bin/env python3
"""
Fixed super-fast PR candidate finder with proper GraphQL pagination.
"""
...
class GraphQLPRFetcher:
    """Fetches COMPLETE PR data using GitHub GraphQL API with proper pagination."""
    ...
    def fetch_prs_bulk(self, pr_numbers: List[int]) -> List[Dict]:
        ...
        # Build aliases for each PR with PROPER PAGINATION
        pr_queries = []
        for i, pr_num in enumerate(pr_numbers[:10]):  # Reduced further to avoid complexity limits
            pr_queries.append(f"""
            pr{i}: pullRequest(number: {pr_num}) {{
                ...
                # Review comments (get_comments() equivalent)
                comments(first: 50) {{
                    totalCount
                    nodes {{
                        body
                ...
                # Issue/conversation comments (get_issue_comments() equivalent)
                ...
                # Reviews (get_reviews() equivalent)
                reviews(first: 50) {{
                    totalCount
                    nodes {{
                        author {{ login }}
                ...
                # Review comments (get_review_comments() equivalent)
                ...
                # PR-level reactions - FIXED WITH PAGINATION
                reactions(first: 50) {{
                    totalCount
                    nodes {{
                        user {{ login }}
    ...
    def fetch_linked_issues(self, pr_body: str, github_client: Github) -> Dict:
        """Fetch reactions from linked issues (issue_reactions() equivalent)."""
        if not pr_body:
...
class CompleteFastPrStat:
    """Complete PR statistics calculator with ALL original functionality."""
    ...
    def calc_comments(self):
        """Process review comments (equivalent to original calc_comments)."""
    ...
    def calc_conv_comments(self):
        """Process conversation/issue comments (equivalent to original calc_conv_comments)."""
        ...
        for item in timeline_nodes:
            ...
            # Count reactions on this conversation comment
    ...
    def calc_review_comments(self):
        """Process review thread comments (get_review_comments equivalent)."""
        ...
        for thread in thread_nodes:
            ...
            for comment in comment_nodes:
                ...
                # These are counted separately in original (part of comment_length calculation)
                self.len_comments += len(comment_body)
    ...
    def calc_reviews(self):
        """Process reviews (equivalent to original num_reviews)."""
    ...
    def calc_issue_reactions(self):
        """Process linked issue data (equivalent to original issue_reactions)."""
        if self.issue_data:
    ...
    def calc_interaction_score(self):
        """Calculate interaction score (equivalent to original)."""
        interactions = (
    ...
    def adjust_interaction_score(self):
        """Apply protm multiplier (equivalent to original)."""
        if self.tagged_protm:
            self.interaction_score_value *= 20

    def process_all_data(self):
        """Process all PR data (equivalent to calling all calc_* methods)."""
        if self._processed:
            return

        # Check for protm in title and body first
    ...
    def length_score(self) -> float:
        ...
        self.process_all_data()  # Ensure data is processed
    ...
    def score(self) -> float:
        """Calculate final score (equivalent to original)."""
        if self._score is not None:
            return self._score

        self.process_all_data()  # Ensure all data is processed
...
class SuperFastPRFinder:
    ...
    def search_prs_with_filters(self, date_start: datetime, date_end: datetime,
        ...
        # Enhanced search queries to find protm PRs more effectively
        search_queries = [
            # High-priority: #protm in any text
        ...
        for query in search_queries:
            ...
            try:
                search_result = self.github_client.search_issues(
                    query=query,
                    sort="updated",
                    order="desc"
                )
                ...
                for issue in search_result:
                    ...
                    if batch_count >= 200:  # Larger batches for efficiency
                        break
    ...
    def quick_score_prs(self, prs: List[Dict]) -> List[Tuple[int, float]]:
        ...
        for pr in prs:
            ...
            # #protm massive boost (check title and body)
    ...
    def fetch_full_pr_data(self, pr_numbers: List[int], max_workers: int = 4) -> List[CompleteFastPrStat]:
        """Fetch COMPLETE PR data with linked issues."""

        console.print(f"[blue]Fetching complete data for {len(pr_numbers)} PRs...[/]")

        # Step 1: Fetch PR data with GraphQL (smaller batches due to pagination complexity)
        batch_size = 8  # Smaller batches to avoid GraphQL complexity issues
        ...
        # Step 2: Fetch linked issue data for PRs that have them
        console.print(f"[blue]Fetching linked issue data...[/]")

        def fetch_issue_data(pr_data):
            try:
                return self.graphql_fetcher.fetch_linked_issues(
                    pr_data.get("body", ""),
                    self.github_client
                )
            except Exception as e:
        ...
        # Step 3: Create CompleteFastPrStat objects
        pr_stats = []
        for pr_data, issue_data in zip(all_pr_data, issue_data_list):
            if pr_data:
                pr_stats.append(CompleteFastPrStat(pr_data, issue_data))
...
def main(
    ...
    load,
    verbose: bool,
    cache_search: Optional[str],
    load_search: Optional[str],
):
    ...
    if load:
        ...
        # Phase 2: Enhanced quick scoring
        ...
        # Phase 3: Complete data fetching with fixed pagination
    ...
    # Results with protm highlighting
    ...
    for i, (pr_num, score) in enumerate(top_final, 1):
        ...
        if pr_stat:
            pr_stat.process_all_data()  # Ensure protm detection is complete
            ...
            if verbose:
                console.print(f"     [dim]Author: {pr_stat.author}, Files: {pr_stat.changed_files}, "
                              f"+{pr_stat.additions}/-{pr_stat.deletions}, Comments: {pr_stat.num_comments + pr_stat.num_conv_comments}[/dim]")
                if pr_stat.tagged_protm:
                    console.print(f"     [magenta]🔥 CONTAINS #PROTM TAG[/magenta]")

    if save:
        console.print(f"[blue]💾 Saving complete results...[/]")
