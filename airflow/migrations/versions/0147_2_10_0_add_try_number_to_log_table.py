...
Revision ID: 48cce4d89237
Revises: d482b7261ff9
Create Date: 2024-07-14 17:05:34.700183
...
def upgrade():
    """Apply add try_number to log table."""
    with op.batch_alter_table("log", schema=None) as batch_op:
        batch_op.add_column(sa.Column("try_number", sa.Integer(), nullable=True))
        batch_op.drop_index("idx_log_dag")
        batch_op.create_index(
            "idx_log_ti_pk", ["dag_id", "task_id", "run_id", "map_index", "try_number"], unique=False
        )


def downgrade():
    """Unapply add try_number to log table."""
    with op.batch_alter_table("log", schema=None) as batch_op:
        batch_op.drop_index("idx_log_ti_pk")
