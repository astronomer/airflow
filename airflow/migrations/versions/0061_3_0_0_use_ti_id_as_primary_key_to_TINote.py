...
def upgrade():
    ...
    with op.batch_alter_table("task_instance_note", schema=None) as batch_op:
        batch_op.add_column(
            sa.Column("id", sa.String(length=36).with_variant(postgresql.UUID(), "postgresql"))
        )
    if dialect_name == "postgresql":
        op.execute("""
            UPDATE task_instance_note SET id = task_instance.id
            FROM task_instance
        ...
        op.execute("""
            ...
            SET id = (SELECT id FROM task_instance WHERE task_instance_note.task_id = task_instance.task_id
            AND task_instance_note.dag_id = task_instance.dag_id
            AND task_instance_note.run_id = task_instance.run_id
            AND task_instance_note.map_index = task_instance.map_index)
            """)
    with op.batch_alter_table("task_instance_note", schema=None) as batch_op:
        ...
        batch_op.create_primary_key("task_instance_note_pkey", ["id"])
        batch_op.create_foreign_key(
            "task_instance_note_ti_fkey", "task_instance", ["id"], ["id"], ondelete="CASCADE"
        )
...
def downgrade():
    ...
    if dialect_name == "postgresql":
        op.execute("""
            ...
            FROM task_instance
            WHERE task_instance_note.id = task_instance.id
            """)
    elif dialect_name == "mysql":
        op.execute("""
            UPDATE task_instance_note tin
        ...
        op.execute("""
            UPDATE task_instance_note
            SET dag_id = (SELECT dag_id FROM task_instance WHERE task_instance_note.id = task_instance.id),
                task_id = (SELECT task_id FROM task_instance WHERE task_instance_note.id = task_instance.id),
                run_id = (SELECT run_id FROM task_instance WHERE task_instance_note.id = task_instance.id),
                map_index = (SELECT map_index FROM task_instance WHERE task_instance_note.id = task_instance.id)
            """)
    with op.batch_alter_table("task_instance_note", schema=None) as batch_op:
        batch_op.drop_constraint("task_instance_note_ti_fkey", type_="foreignkey")
        batch_op.drop_constraint("task_instance_note_pkey", type_="primary")
