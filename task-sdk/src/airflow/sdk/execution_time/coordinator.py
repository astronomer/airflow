...
class PythonCoordinator(BaseCoordinator):
    """
...
@functools.cache
def _build_python_coordinator() -> PythonCoordinator:
    return PythonCoordinator()
...
class CoordinatorManager:
    ...
    def from_config(cls) -> Self:
        ...
        coordinator_entries = conf.getjson("sdk", "coordinators", fallback={})
        if not isinstance(coordinator_entries, dict):
            coordinator_entries = {}
        ...
        queue_to_coordinator = {
            queue: _build_coordinator(coordinator_key) for queue, coordinator_key in queue_mapping.items()
        }
