...
PRAGMA, but the safe rule is strictly stronger) is documented in
``contributing-docs/26_migration_round_trip_check.rst``.

For the placement convention every migration must follow and the FK
chains exercised by the seed, see
``contributing-docs/26_migration_round_trip_check.rst``.
"""
...
def _seed_for_current_schema() -> None:
    ...
    with settings.get_engine().begin() as conn:
        ...
        ti_cols = {r[1] for r in _table_info(conn, "task_instance")} if _table_exists(conn, "task_instance") else set()
...
def main() -> None:
    ...
    try:
        ...
        _SKIP_EXTERNAL_DB_MANAGERS_UPGRADE.set(True)

        _step(f"Walk UP base → {revs[-1][:8]}…")
        for rev in revs:
            try:
                _seed_for_current_schema()
