...
class SSHHookAsync(BaseHook):
    ...
    def _parse_extras(self, conn: Any) -> None:
        ...
        # Only override known_hosts from extras if it wasn't explicitly set in __init__
        # (check if it's still the default value after expansion)
        ...
        # If no_host_key_check is not set in extras, default to False (check host key)
        # If host_key is provided, it implies we should check it
