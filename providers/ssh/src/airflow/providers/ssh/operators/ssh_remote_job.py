...
class SSHRemoteJobOperator(BaseOperator):
    ...
    def _validate_base_dir(path: str) -> None:
        ...
        # Warn about potentially dangerous paths (but don't block - they might be intentional)
    ...
    def _detect_remote_os(self) -> Literal["posix", "windows"]:
        ...
        with self.ssh_hook.get_conn() as ssh_client:
            # Try POSIX detection first (most common)
            try:
            ...
            # Both failed - raise error instead of assuming
    ...
    def execute(self, context: Context) -> None:
        ...
        with self.ssh_hook.get_conn() as ssh_client:
            exit_status, stdout, stderr = self.ssh_hook.exec_ssh_client_command(
                ...
                get_pty=False,
                environment=None,
                timeout=60,  # Submission should be quick
            )
        ...
        # Store job info in XCom for debugging/monitoring
    ...
    def execute_complete(self, context: Context, event: dict[str, Any]) -> None:
        ...
        if not event.get("done", False):
            # Job still running, defer again with updated offset
    ...
    def on_kill(self) -> None:
        ...
        we retrieve job information from XCom if instance variables are not available.
        """
        # Try instance variables first (if still in execute())
        job_id = self._job_id
        pid_file = self._paths.pid_file if self._paths else None
        remote_os = self._detected_os

        # If not available (rehydrated after deferral), get from task instance XCom
        if not job_id or not pid_file or not remote_os:
            try:
                # Access via task_instance if available
