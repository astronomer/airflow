...
class SSHRemoteJobTrigger(BaseTrigger):
    ...
    async def _read_log_chunk(self, hook: SSHHookAsync) -> tuple[str, int]:
        ...
        """
        # First get the file size to know how much is available
        ...
        try:
            ...
            # For POSIX, dd returns the actual bytes read; for Windows, we read the string
            # Calculate new offset based on bytes actually read from remote
            # The remote command reads bytes_to_read bytes from the file
            # We advance offset by the amount we requested, not by the decoded string length
            if stdout:
                new_offset = self.log_offset + bytes_to_read
            else:
                # No data read (file may have been truncated or deleted)
                new_offset = self.log_offset
    ...
    async def run(self) -> AsyncIterator[TriggerEvent]:
        ...
        while True:
            try:
                ...
                # Read any available log output
                log_chunk, new_offset = await self._read_log_chunk(hook)

                # Build base event data with all path information for potential re-deferral
                base_event = {
                    "job_id": self.job_id,
                    "job_dir": self.job_dir,
                    "log_file": self.log_file,
                ...
                # Job still running, update offset for next poll
                self.log_offset = new_offset

                # Job still running, yield current state
                yield TriggerEvent(
                    {
                        **base_event,
                        "status": "running",
