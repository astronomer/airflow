...
class TestSSHRemoteJobOperator:
    def setup_method(self):
        ...
        self.hook.remote_host = "test.host.com"

        ssh_client = mock.create_autospec(SSHClient)
        ssh_client.__enter__ = mock.MagicMock(return_value=ssh_client)
        ssh_client.__exit__ = mock.MagicMock(return_value=None)
        self.hook.get_conn = mock.MagicMock(return_value=ssh_client)
    ...
    @mock.patch.object(SSHRemoteJobOperator, "ssh_hook", new_callable=mock.PropertyMock)
    def test_execute_defers_to_trigger(self, mock_hook_prop):
        """Test that execute submits job and defers to trigger."""
        mock_hook = mock.MagicMock(spec=SSHHook)
        mock_hook.remote_host = "test.host.com"
        mock_hook.get_conn.return_value.__enter__ = mock.MagicMock(return_value=mock.MagicMock())
        mock_hook.get_conn.return_value.__exit__ = mock.MagicMock(return_value=None)
        mock_hook.exec_ssh_client_command.return_value = (
            0,
    ...
    @mock.patch.object(SSHRemoteJobOperator, "ssh_hook", new_callable=mock.PropertyMock)
    def test_execute_complete_with_cleanup(self, mock_hook_prop):
        ...
        mock_hook.exec_ssh_client_command.assert_called_once()
        call_args = mock_hook.exec_ssh_client_command.call_args
        assert "rm -rf" in call_args[0][1]

    @mock.patch.object(SSHRemoteJobOperator, "ssh_hook", new_callable=mock.PropertyMock)
    def test_on_kill(self, mock_hook_prop):
    ...
    @mock.patch.object(SSHRemoteJobOperator, "ssh_hook", new_callable=mock.PropertyMock)
    def test_on_kill_after_rehydration(self, mock_hook_prop):
