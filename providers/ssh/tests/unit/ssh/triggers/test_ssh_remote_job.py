...
class TestSSHRemoteJobTrigger:
    ...
    @pytest.mark.asyncio
    async def test_run_job_still_running(self):
        """Test trigger when job is still running."""
        trigger = SSHRemoteJobTrigger(
            ssh_conn_id="test_conn",
            remote_host=None,
            job_id="test_job",
        ...
        with mock.patch.object(trigger, "_check_completion", return_value=None):
            with mock.patch.object(trigger, "_read_log_chunk", return_value=("Some output\n", 50)):
                events = []
                async for event in trigger.run():
                    events.append(event)
                ...
                assert events[0].payload["log_offset"] == 50
