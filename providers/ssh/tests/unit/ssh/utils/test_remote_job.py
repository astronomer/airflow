...
class TestRemoteJobPaths:
    ...
    def test_windows_default_paths(self):
        ...
        assert paths.base_dir == "C:\\Windows\\Temp\\airflow-ssh-jobs"
        assert paths.job_dir == "C:\\Windows\\Temp\\airflow-ssh-jobs\\test_job"
        assert paths.log_file == "C:\\Windows\\Temp\\airflow-ssh-jobs\\test_job\\stdout.log"
        assert paths.exit_code_file == "C:\\Windows\\Temp\\airflow-ssh-jobs\\test_job\\exit_code"
        assert paths.pid_file == "C:\\Windows\\Temp\\airflow-ssh-jobs\\test_job\\pid"
        assert paths.sep == "\\"
...
class TestBuildWindowsWrapperCommand:
    def test_basic_command(self):
        ...
        assert "New-Item -ItemType Directory" in wrapper
...
class TestCleanupCommands:
    def test_posix_cleanup(self):
        ...
        cmd = build_posix_cleanup_command("/tmp/job_dir")
        assert "rm -rf" in cmd
        assert "/tmp/job_dir" in cmd

    def test_windows_cleanup(self):
        """Test Windows cleanup command."""
        cmd = build_windows_cleanup_command("C:\\temp\\job_dir")
