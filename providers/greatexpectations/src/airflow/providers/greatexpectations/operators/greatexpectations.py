...
try:
    ...
    from great_expectations.data_context.types.base import (  # type: ignore[attr-defined]
        CheckpointConfig,
        DataContextConfig,
    )
...
class GreatExpectationsDataDocsLink(BaseOperatorLink):
    """Construct a link to Great Expectations data docs site."""
...
class GreatExpectationsOperator(BaseOperator):
    ...
    This operator is meant to be used for GX Core (great-expectations python library) < 1.0
    ...
    def __init__(
        ...
        data_context_root_dir: str | bytes | os.PathLike | None = None,
        data_context_config: DataContextConfig | None = None,
        dataframe_to_validate: DataFrame | None = None,  # should we allow a Spark DataFrame as well?
        query_to_validate: str | None = None,
        checkpoint_name: str | None = None,
        checkpoint_config: CheckpointConfig | None = None,
        checkpoint_kwargs: dict[str, Any] | None = None,
        validation_failure_callback: Callable[[CheckpointResult], None] | None = None,
        ...
        **kwargs: Any,
        ...
        self.dataframe_to_validate = dataframe_to_validate
        self.query_to_validate = query_to_validate
        self.checkpoint_name = checkpoint_name
        self.checkpoint_config = checkpoint_config if checkpoint_config else {}
    ...
    def make_connection_configuration(self) -> dict[str, str]:
        """Build connection strings based off existing Airflow connections. Only supports necessary extras."""
        uri_string = ""
        driver = ""
        if not self.conn:
        ...
        if conn_type in ("redshift", "mysql", "mssql"):
            ...
            if self.conn.schema:
                ...
                odbc_connector = "postgresql+psycopg2"
                uri_string = f"{odbc_connector}://{self.conn.login}:{self.conn.password}@{self.conn.host}:{self.conn.port}/{postgres_database}"
            else:
                raise ValueError(
            ...
            snowflake_warehouse = (
                self.conn.extra_dejson.get("warehouse")
                or self.conn.extra_dejson["extra__snowflake__warehouse"]
            )
            snowflake_role = (
                self.conn.extra_dejson.get("role") or self.conn.extra_dejson["extra__snowflake__role"]
            )

            if snowflake_region:
                uri_string = f"snowflake://{self.conn.login}:{self.conn.password}@{snowflake_account}.{snowflake_region}/{snowflake_database}/{self.schema}?warehouse={snowflake_warehouse}&role={snowflake_role}"
            else:
                uri_string = f"snowflake://{self.conn.login}:{self.conn.password}@{snowflake_account}/{snowflake_database}/{self.schema}?warehouse={snowflake_warehouse}&role={snowflake_role}"
            ...
            if athena_db:
                uri_string = (
                    f"awsathena+rest://@athena.{region}.amazonaws.com/{athena_db}?s3_staging_dir={s3_path}"
                )
            else:
    ...
    def build_snowflake_connection_config_from_hook(self) -> dict[str, Any]:
        ...
        if not self.conn_id:
            raise ValueError("conn_id must be set to build Snowflake connection config")
    ...
    def build_runtime_datasources(self):
        """Build datasources at runtime based on Airflow connections or for use with a dataframe."""
    ...
    def build_default_action_list(self) -> list[dict[str, Any]]:
        """Build a default action list for a default checkpoint."""
        action_list = [
            {
    ...
    def build_default_checkpoint_config(self):
        """Build a default checkpoint with default values."""
    ...
    def execute(self, context: Context) -> CheckpointResult | dict[str, Any]:
        """Determine whether a checkpoint exists or need to be built, then runs the resulting checkpoint."""
        ...
        if self.data_context_root_dir:
            self.data_context = ge.data_context.FileDataContext(
                context_root_dir=self.data_context_root_dir,  # type: ignore[arg-type]
        ...
        if self.checkpoint_name:
            self.checkpoint = self.data_context.get_checkpoint(name=self.checkpoint_name)  # type: ignore[attr-defined]
        ...
        if self.return_json_dict:
            return result.to_json_dict()  # type: ignore[attr-defined]
        ...
        if not result["success"]:
            ...
            if self.fail_task_on_validation_failure:
                ...
                for _, value in result.run_results.items():
                    ...
                    result_information["batch_definition"] = value["validation_result"].meta[
                ...
                if len(result_list) < 3:
                    result_list = result_list[0]  # type: ignore[assignment]

                raise AirflowException(f"Validation with Great Expectations failed.\nResults\n {result_list}")
            self.log.warning(
