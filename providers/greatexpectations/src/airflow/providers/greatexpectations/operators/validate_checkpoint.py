...
class GXValidateCheckpointOperator(BaseOperator):
    ...
    :param configure_checkpoint: A callable that returns a Checkpoint, which orchestrates a ValidationDefinition,
    ...
    :param context_type: accepts `ephemeral`, `cloud`, or `file` to set the DataContext used by the Operator.
    ...
    :param configure_file_data_context: callable that returns or yields a `FileDataContext`. Applicable only when using
