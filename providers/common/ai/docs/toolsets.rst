...
- **It does not contain the agent.** The agent loop, the model calls, and every
  ...
  that can reach connections has a contained code tool sitting next to an
  uncontained credential path. The sandbox does not change what those other
  tools can do.
...
.. list-table::
   ...
   * - You want
     - Use
   * - The agent to analyse data, install a package, run a script it wrote, or
...
The ``sbx`` backend, for instance, refuses ``allow_egress_to`` outright, because
Docker Sandboxes governs egress with a host-level policy rather than a
per-sandbox rule.
...
.. warning::
   ...
   **Use this backend for local development, not production.** Docker Sandboxes
   ...
   ``sbx policy init``, and on Linux, KVM or nested virtualisation. A worker in
...
sandbox in an `islo.dev <https://islo.dev>`__ microVM. There is no local daemon
and no host virtualisation requirement, so unlike ``sbx`` this works from a
worker running in a container, which makes it the backend to reach for on
Kubernetes.
...
The command timeout is enforced by the backend, not the islo API, whose
``timeout_secs`` is only a hint. If no terminal command state arrives within the
timeout plus a short grace period, the backend deletes the microVM, marks the
sandbox terminated and reports a timeout.

Bringing your own backend
^^^^^^^^^^^^^^^^^^^^^^^^^

Any vendor that can create a sandbox, run a command in it, move files and
destroy it can plug in. Subclass
:class:`~airflow.providers.common.ai.sandbox.SandboxBackend` in your own package
and pass an instance to ``SandboxToolset``. Four of the six methods are named
after the four tools, so the mapping is literal:
...
.. code-block:: python
    ...
    class AcmeSandboxBackend(SandboxBackend):
        ...
        def write_file(self, sandbox, path, content: bytes) -> None: ...

        def list_directory(self, sandbox, path) -> list[tuple[str, bool]]: ...
...
.. list-table::
   ...
   * - **SandboxToolset: off-worker execution**
     ...
     - **Does not contain the agent.** The agent loop and every other toolset on
       ...
       backend cannot apply per-sandbox network rules and leaks orphaned microVMs
