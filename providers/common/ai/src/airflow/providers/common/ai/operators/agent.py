...
class AgentOperator(BaseOperator, HITLReviewMixin):
    ...
    **HITL Review parameters** (requires the ``hitl_review`` plugin):
    ...
    # Appended to a reviewer's feedback when the agent has memory. A human correcting the
    # output is the only real signal this agent gets about whether it was any good: left to
    # introspect on its own, it has nothing to check its guesses against and mostly restates
    ...
    def _resolve_agent(self) -> Any:
        ...
        if self._memory_active:
            ...
            self.toolsets = [
                *(self.toolsets or []),
                build_memory_toolset(resolved, self.MEMORY_KEY, self.log, context=resolved.context),
            ]
    ...
    def _record_agent_run(self, resolved: Any, result: Any) -> None:
        ...
        """
        try:
            # `usage.cost` is best-effort and None when the provider reports no pricing.
            # It is a Decimal, which is not JSON representable, hence the float().
    ...
    def regenerate_with_feedback(self, *, feedback: str, message_history: Any) -> tuple[str, Any]:
        ...
        When the agent has memory, the reviewer's correction is also an opportunity to learn:
        the `remember` toolset is already attached, so the rewrite and the lesson come out of
        the same call rather than costing an extra one.
        """
        usage_limits = coerce_usage_limits(self.usage_limits)
        agent = self._build_agent()
