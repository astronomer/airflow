...
The XCom tab shows the agent's markdown as one escaped string. This renders it, alongside
the standing context and learned lessons that shaped it.
...
def render_markdown(text: str) -> str:
    ...
    in_code = False
    paragraph: list[str] = []
    # Open lists, outermost first, as (indent, tag). A stack because models write sub-bullets
    # under numbered points, and treating those as a sibling list restarts numbering at 1.
        ...
        external_views = [
            {
                ...
                # Required in practice, though optional in the type: usePluginTabs filters out
                # any view without one, so a tab with no url_route never renders.
                "url_route": "agent-review",
