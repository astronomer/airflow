...
class SbxSandboxBackend(SandboxBackend):
    ...
    **Use this for local development, not production.** Docker Sandboxes is
    ...
    virtualisation -- which an unprivileged container cannot provide. No hosted
    ...
    **Network policy is a host-level setting, not a per-sandbox one.** ``sbx``
    ...
    force when it is not, ``create`` refuses a spec it cannot honour unless the
    ...
    **Orphans are not reclaimed automatically.** There is no server-side TTL to
    ...
    def _check_spec(self, spec: SandboxSpec | None) -> None:
        ...
        if spec.allow_egress_to:
            raise SandboxTerminalError(
                "The sbx backend cannot apply a per-sandbox egress allowlist: Docker Sandboxes "
                "governs egress with the host-level 'sbx policy'. Drop allow_egress_to and set "
                "the policy on the host with 'sbx policy'. No backend shipping with this "
                "provider supports per-sandbox egress rules."
            )
    ...
    def run_command(
        ...
        try:
            ...
            # The CLI never returned: the command may still be running in the shared
            # microVM. Destroy it so it cannot continue, and tell the toolset to
    ...
    def _file_helper(
        ...
        and an uncapped read would exhaust worker memory and take the process
        down with it.
        """
        # -i ("keep STDIN open even if not attached") only when there is a
        # payload. Verified against sbx 0.38.0: with stdin attached through a
        # pipe the payload arrives either way, so this is belt-and-braces for a
        # worker invoked without a stdin of its own, not a fix for data loss.
        exec_args = ["exec", "-i", sandbox] if stdin is not None else ["exec", sandbox]
        code, out, err, _, _ = self._exec_capped_bytes(
            [*exec_args, "sh", "-c", script],
            timeout=_FILE_OP_TIMEOUT,
            max_output_bytes=max_output_bytes,
            stdin=stdin,
        )
        if code != 0:
            raise SandboxError(err.decode(errors="replace").strip() or "command failed")
        return out

    def read_file(self, sandbox: str, path: str, *, max_bytes: int) -> bytes:
        quoted = shlex.quote(path)
        # One exec, and the cap is enforced guest-side by `head -c`. Checking the
        ...
        if len(data) > max_bytes:
            # `head` handed back the sentinel byte, so the file is larger than the
            # budget. Prefer stat's number when it looks sane; a streaming source
            # reports 0, in which case the true size is unknown but irrelevant.
            try:
    ...
    def write_file(self, sandbox: str, path: str, content: bytes) -> None:
        quoted = shlex.quote(path)
        # Content goes over stdin rather than argv so a large file cannot exceed
