...
Migrations that rebuild a parent table via ``op.batch_alter_table`` must
wrap their entire body in ``disable_sqlite_fkeys(op)`` *before* any DML or
DDL opens an implicit transaction — otherwise the wrapper's PRAGMA is a
no-op and the rebuild's implicit ``DROP TABLE`` cascade-deletes child rows
(or aborts on a RESTRICT chain). The placement convention, the
per-airflow-version round-trip prek hook that enforces it, and the
baseline file used to tolerate already-known placement bugs are documented
in `Migration round-trip regression check <26_migration_round_trip_check.rst>`__.
