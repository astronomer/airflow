...
.. code-block:: bash
    ...
    # Lint JS/TS code (Oxlint, then ESLint for the rules Oxlint has no equivalent
    # for, then tsc) and report any errors/warnings
    pnpm lint

    # Same as above, fixing whatever can be fixed automatically
    pnpm lint:fix
...
- ``/rules`` ESLint rules for javascript and typescript code that Oxlint has no
  equivalent for; everything else lives in ``.oxlintrc.json``
