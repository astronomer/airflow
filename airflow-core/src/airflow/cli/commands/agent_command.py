...
def agent_remember(args, session: Session = NEW_SESSION):
    ...
    An agent's own lessons are drawn from the same material it was already given, so they
    tend to restate it. The lessons that actually change an answer are facts it could not
    derive -- a project convention, a naming rule, where a file has to go. Those come from
    a person.
    """
    agent = _fetch_agent(args.name, session)
