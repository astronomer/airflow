...
worker when a RetryPolicy is configured.  They are read by
next_retry_datetime() and cleared when the task enters RUNNING.
...
def upgrade():
    """Add retry policy fields to task_instance."""
...
def downgrade():
    """Remove retry policy fields from task_instance."""
    with op.batch_alter_table("task_instance", schema=None) as batch_op:
