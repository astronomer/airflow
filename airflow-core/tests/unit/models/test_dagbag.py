...
class TestDBDagBag:
    ...
    def test__read_dag_stores_and_returns_dag(self):
        ...
        assert self.db_dag_bag._dags["v1"] == (mock_dag, "hash1")
    ...
    def test_get_dag_returns_cached_on_hit_when_hash_matches(self):
        """A cache hit whose hash still matches the DB is served without a full reload."""
        mock_dag = MagicMock(spec=SerializedDAG)
    ...
    def test_get_dag_reloads_when_version_updated_in_place(self):
        ...
        self.session.get.assert_called_once()
        assert self.db_dag_bag._dags["v1"] == (fresh_dag, "new_hash")
...
class TestDBDagBagCache:
    ...
    def test_cache_hit_metric_emitted(self, mock_stats):
        ...
        dag_bag = DBDagBag(cache_size=10, cache_ttl=60)
        mock_session = MagicMock()
        dag_bag._dags["test_version"] = _CacheEntry(MagicMock(), "hash1")
        # Validation lookup returns the same hash, so the entry is a valid hit.
