...
class TestAssetModelOperation:
    ...
    def test_new_assets_are_inserted_in_one_order_whatever_defined_them(self, session):
        """
        ``asset`` is unique on (name, uri), so two writers inserting the same new assets in
        opposite orders deadlock on that index. A batched write carries several files' assets at
        once, so the order they arrive in is no longer one file's business.
        """
    ...
    def test_sorting_for_insertion_does_not_change_which_asset_is_activated(self, session):
        ...
        only the one handed over first is activated. That is decided by the order the Dags define
        them, and sorting the rows for insertion must not quietly move it.
        """
        op = AssetModelOperation.collect(
            {
